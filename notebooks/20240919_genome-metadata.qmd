...
plot.tbl <- joined.tbl %>% filter(genome_drug.evidence == "Laboratory Method") %>%
  select(genome.genome_id, genome_drug.antibiotic, genome_drug.resistant_phenotype, genome_drug.laboratory_typing_method, genome.isolation_country, genome.collection_year, genome.host_common_name, id_input.type, genome.species) %>% 
  distinct()
